...
async def get_video_from_zephyrflick_player(player_url: str, preferred_lang: str = None):
    ...
    try:
        ...
        api_url = f"https://play.zephyrix.org/player/index.php"  # host moved from zephyrflick.top
        params = {
            'data': video_id,
            'do': 'getVideo'
        }
        ...
        if preferred_lang:
            video_url = video_url.replace('https://play.zephyrix.org', f'{Config.PROTOCOL}://{Config.REDIRECT_URL}/{preferred_lang}')
        else:
            video_url = video_url.replace('https://play.zephyrix.org', f'{Config.PROTOCOL}://{Config.REDIRECT_URL}')
